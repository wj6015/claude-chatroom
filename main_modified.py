...
class PageStore:
    ...
    def append_new_unique(self, page_num, rows, key_func):
        """
        二次轮询补录时使用：只追加当前总结果中尚未出现过的38字段唯一记录。
        这样不会因为重抓同一页，把第一轮已经抓到的记录重复塞入结果；
        但如果第二轮发现了第一轮漏掉的新唯一记录，会追加保留。
        """
        page_num = int(page_num)
        if page_num not in self.page_rows:
            self.page_rows[page_num] = []

        existed = set()
        for page_rows in self.page_rows.values():
            for row in page_rows:
                existed.add(key_func(row))
        ...
        for row in rows:
            ...
            if k in existed:
                continue
            self.page_rows[page_num].append(row)
            existed.add(k)
            added += 1
...
"""
    ...
    def unique_count(self):
        """按38字段唯一键统计当前已采集到的唯一数据量。"""
        rows = self.page_store.flatten()
        return len(set(self.record_key(r) for r in rows))

    def append_new_unique_page(self, page_num, records):
        """二次轮询时补录新唯一记录，不覆盖第一轮页面数据。"""
        return self.page_store.append_new_unique(page_num, records, self.record_key)
    ...
    def second_round_full_poll(self, max_pages):
        """
        当第一轮完成后，38字段唯一数据量小于目标总条数时，
        再从第1页到最后1页完整轮询一遍。

        注意：第二轮不覆盖第一轮数据，只补录第一轮没有采到的新唯一记录。
        这样既能补漏，又不会因为重抓页面导致导出结果人为重复膨胀。
        """
        self.log(f"唯一数据量 {self.unique_count()} 小于目标总条数 {self.total_count}，启动第二轮全量兜底轮询。")
        ...
        for page_num in range(1, max_pages + 1):
            ...
            before_unique = self.unique_count()
            self.log(f"[第二轮] 跳转到第 {page_num} 页 ...")
            self.clear_performance_logs()

            # 第二轮第1页也要主动触发一次跳页/刷新请求，不能只依赖旧响应。
            if not self.goto_page(page_num):
                self.log(f"[第二轮] 第 {page_num} 页翻页失败，尝试重试一次。")
                self.clear_performance_logs()
                if not self.goto_page(page_num):
                    self.log(f"[第二轮] 第 {page_num} 页仍然失败，跳过该页。")
                    continue

            if not self.wait_for_page_num(page_num, timeout=PAGE_CHANGE_TIMEOUT):
                self.log(f"[第二轮] 第 {page_num} 页页码未确认变化，继续等待网络响应。")

            time.sleep(PAGE_SETTLE_DELAY)

            self.log(f"[第二轮] 等待第 {page_num} 页 {TARGET_API_KEYWORD} 响应体可读 ...")
            payload = self.wait_for_latest_query_payload(timeout=NETWORK_RESPONSE_TIMEOUT)

            if not payload:
                self.log(f"[第二轮] 第 {page_num} 页响应超时，重试一次。")
                self.clear_performance_logs()
                if not self.goto_page(page_num):
                    self.log(f"[第二轮] 第 {page_num} 页二次翻页失败，跳过。")
                    continue
                payload = self.wait_for_latest_query_payload(timeout=NETWORK_RESPONSE_TIMEOUT)

            if not payload:
                self.log(f"[第二轮] 第 {page_num} 页仍未抓到响应，跳过。")
                continue

            records = self.payload_to_rows(payload)
            added = self.append_new_unique_page(page_num, records)
            after_unique = self.unique_count()

            self.log(
                f"[第二轮 P{page_num}] 抓到 {len(records)} 条，新增唯一 {added} 条，"
                f"唯一累计 {after_unique} / {self.total_count}，总行数 {self.page_store.total_rows()}"
            )
            self.gui.log_queue.put(("STATUS", page_num, after_unique, self.total_count))

            if after_unique >= self.total_count:
                self.log("第二轮已补齐目标唯一数据量，提前结束兜底轮询。")
                break

            if after_unique == before_unique:
                # 只写日志，不改变任何节奏和逻辑。
                self.log(f"[第二轮 P{page_num}] 本页未发现新的唯一记录。", "debug")
    ...
    def collect(self):
        ...
        try:
            ...
            unique_after_first = self.unique_count()
            self.log(f"第一轮完成：总行数 {self.page_store.total_rows()} 条，38字段唯一数据量 {unique_after_first} / {self.total_count}")

            if (not self.stop_event.is_set()) and unique_after_first < self.total_count:
                self.second_round_full_poll(max_pages)
                unique_after_second = self.unique_count()
                self.log(f"第二轮完成：总行数 {self.page_store.total_rows()} 条，38字段唯一数据量 {unique_after_second} / {self.total_count}")
            else:
                self.log("第一轮唯一数据量已达到目标总条数，直接导出。")
